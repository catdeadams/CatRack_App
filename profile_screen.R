...
setup_profile_server <- function(input, output, session, rv) {
  ...
  # NOTE: the former `input$save_profile` observer (full goal/difficulty/
  # split/equipment edit + program regeneration) was removed — the profile
  # UI replaced it with the dedicated Edit Gear / Edit Frequency modals
  # above, so it had no trigger and was dead code. Goal/difficulty/split
  # changes now go through "+ New Program" onboarding.
}
